 ...
 */
export async function getVacantSeats(batchYear = undefined, branch = undefined) {
	try{
		const stages: any = [
			{
				'$lookup': {
				...
				'$group': {
					...
					'departments': {
						'$push': {
							...
							'availableIntake': {
								'$subtract': [
									'$department.totalStudentsIntake', '$totalStudents'
								]
							}
						}
					}
